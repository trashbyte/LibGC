...
namespace LibGC.ModelLoader
{
    ...
    class ObjMtlLoader
    {
        ...
        private void ParseObjVertexPositionDeclaration()
        {
            ...
            // The list can have 3 (x,y,z) or 4 (x,y,z,w) components. 
            if (position.Count != 3 && position.Count != 4)
            {
                throw new InvalidObjMtlFileException(string.Format(
                    "{0}: Expected vertex position to contain three numbers.", objParser.GetFilePositionStr()));
            }

            vertexPositions.Add(new Vector4(
                position[0],
                position[1],
                position[2],
                (position.Count == 4) ? position[3] : 1.0f));
        }
        ...
        private void ParseObjVertexTexCoordDeclaration()
        {
            ...
            // The list can have 2 (u,v) or 3 (u,v,w) components. 
            if (texCoord.Count != 2 && texCoord.Count != 3)
            {
                throw new InvalidObjMtlFileException(string.Format(
                    "{0}: Expected texture coordinate to contain two or three numbers.", objParser.GetFilePositionStr()));
            }

            vertexTexCoords.Add(new Vector3(
                texCoord[0],
                texCoord[1],
                (texCoord.Count == 3) ? texCoord[2] : 0.0f));
        }
        ...
        private void ParseObjFaceDeclaration()
        {
            ...
            for (string word = objParser.GetNextWord(); word != null; word = objParser.GetNextWord())
            {
                ...
                ObjMtlVertex vtx = new ObjMtlVertex(
                    ...
                    (vertexTexCoordIdx != -1) ? (Vector3?)vertexTexCoords[vertexTexCoordIdx - 1] : null);
