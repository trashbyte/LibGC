...
namespace LibGC.Tpl
{
	...
	public class TplTexture : ITexture {
        static readonly IReadOnlyCollection<GcTextureFormat> supportedTextureFormatsBackingField = new List<GcTextureFormat> {
			GcTextureFormat.I4,
			GcTextureFormat.I8,
			GcTextureFormat.IA4,
			GcTextureFormat.RGB565,
			GcTextureFormat.RGB5A3,
			GcTextureFormat.RGBA8,
			GcTextureFormat.CMPR
		}.AsReadOnly();

		/// <summary>Get the list of formats supported for textures.</summary>
		public static IReadOnlyCollection<GcTextureFormat> SupportedTextureFormats
		{
			get
			{
        ...
        public TplTexture(GcTextureFormat format, Bitmap bmp)
        {
		...
		public void DefineMainLevel(GcTextureFormat newFormat, int newWidth, int newHeight,
        ...
        public void DefineMainLevelFromBitmap(GcTextureFormat newFormat, Bitmap bmp)
        {
            if (!SupportedTextureFormats.Contains(newFormat))
                throw new ArgumentOutOfRangeException("newFormat", "Unsupported format.");
            if (bmp == null)
        ...
        public void DefineTextureFromBitmap(GcTextureFormat format, Bitmap bmp)
        {
		...
		/// </summary>
		internal void LoadTextureData(EndianBinaryReader input, GcGame game, GcTextureFormat format, int totalLevelSize, int width, int height, int levelCount)
		{
			if (!SupportedTextureFormats.Contains(format))
				throw new InvalidTplFileException("Unsupported texture format.");

            int remainingSize = totalLevelSize;
			...
			for (int level = 0; level < levelCount; level++)
			{
                ...
                // Try to read as many bytes as we can into the level data buffer,
                // but we may not be able to due to bugs in F-Zero GX / SMB2.
                int sizeToRead = Math.Min(levelData.Length, remainingSize);
                input.Read(levelData, 0, sizeToRead);
                remainingSize -= sizeToRead;

				encodedLevelData.Add(levelData);
			}

            if (TplVersionDetails.VerifyCorrectSize(game) && SizeOfTextureData(game) != totalLevelSize)
                throw new InvalidTplFileException("Texture size doesn't match expected size.");
		}

		/// <summary>
		/// Writes this texture to a binary stream.
		/// </summary>
		internal void SaveTextureData(EndianBinaryWriter output, GcGame game)
		{
            int remainingBufferSize = SizeOfTextureData(game);

			for (int level = 0; level < LevelCount; level++)
			{
                int sizeToWrite = Math.Min(remainingBufferSize, encodedLevelData[level].Length);
                output.Write(encodedLevelData[level], 0, sizeToWrite);
                remainingBufferSize -= sizeToWrite;
			}
		}
