...
namespace LibGC.Gma
{
    public class GcmfTriangleStripGroup : NonNullableCollection<GcmfTriangleStrip>
    {
        ...
        internal void LoadIndexed(EndianBinaryReader input, bool is16Bit, int size, OrderedSet<GcmfVertex> vertexPool, uint vertexFlags)
        {
            for (int nIntsRead = 0; nIntsRead < size; )
            {
                GcmfTriangleStrip strip = new GcmfTriangleStrip();
                nIntsRead += strip.LoadIndexed(input, is16Bit, vertexPool, vertexFlags);
                Items.Add(strip);
            }
        }
        ...
        {
            return Items.Sum(strip => strip.NumIntsOfIndexed());
        }

        internal int SizeOfIndexed(bool is16Bit)
        {
            return Items.Sum(strip => strip.SizeOfIndexed(is16Bit));
        }

        internal void SaveIndexed(EndianBinaryWriter output, bool is16Bit, Dictionary<GcmfVertex, int> vertexPool)
        {
            foreach (GcmfTriangleStrip strip in Items)
                strip.SaveIndexed(output, is16Bit, vertexPool);
        }
    }
}
