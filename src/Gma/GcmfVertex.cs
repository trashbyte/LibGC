...
namespace LibGC.Gma
{
    public class GcmfVertex
    {
        ...
        private static float ReadNumberOfType(EndianBinaryReader input, bool is16Bit)
        {
            if (is16Bit)
            {
                // This is definitely the correct scaling factor.
                // Checked by comparing m_init.gma>ICOSA_BALL in SMB1 and SMB2,
                // which contain the same data, but encoded as 16-bit in SMB2.
        ...
        internal void LoadIndexed(EndianBinaryReader input, bool is16Bit)
        {
            // TODOXXX this with type = 8
            indexedVertexPendingFlagsAssignment = true;

            Position = new Vector3(ReadNumberOfType(input, is16Bit),
                ReadNumberOfType(input, is16Bit), ReadNumberOfType(input, is16Bit));
            // NOTE: this is not always an unit vector like on non-indexed ones!
            Normal = new Vector3(ReadNumberOfType(input, is16Bit),
                ReadNumberOfType(input, is16Bit), ReadNumberOfType(input, is16Bit));
            PrimaryTexCoord = new Vector2(ReadNumberOfType(input, is16Bit), ReadNumberOfType(input, is16Bit));
            SecondaryTexCoord = new Vector2(ReadNumberOfType(input, is16Bit), ReadNumberOfType(input, is16Bit));
            TertiaryTexCoord = new Vector2(ReadNumberOfType(input, is16Bit), ReadNumberOfType(input, is16Bit));
            ...
            colorIndexedRaw = UnpackColorRGBA(input.ReadUInt32()); // TODO byte order is different in 16 bit?
            unknown7_1 = input.ReadUInt32(); // TODO

            if (!is16Bit) // Those fields are not present in the 16 bit version
            {
        ...
        {
            return !is16Bit ? 0x40 : 0x20;
        }

        internal void SaveIndexed(EndianBinaryWriter output, bool is16Bit)
        {
            WriteNumberOfType(output, is16Bit, Position.X);
            WriteNumberOfType(output, is16Bit, Position.Y);
            WriteNumberOfType(output, is16Bit, Position.Z);

            WriteNumberOfType(output, is16Bit, Normal.HasValue ? Normal.Value.X : 0.0f);
            WriteNumberOfType(output, is16Bit, Normal.HasValue ? Normal.Value.Y : 0.0f);
            WriteNumberOfType(output, is16Bit, Normal.HasValue ? Normal.Value.Z : 0.0f);

            WriteNumberOfType(output, is16Bit, PrimaryTexCoord.HasValue ? PrimaryTexCoord.Value.X : 0.0f);
            WriteNumberOfType(output, is16Bit, PrimaryTexCoord.HasValue ? PrimaryTexCoord.Value.Y : 0.0f);

            WriteNumberOfType(output, is16Bit, SecondaryTexCoord.HasValue ? SecondaryTexCoord.Value.X : 0.0f);
            WriteNumberOfType(output, is16Bit, SecondaryTexCoord.HasValue ? SecondaryTexCoord.Value.Y : 0.0f);

            WriteNumberOfType(output, is16Bit, TertiaryTexCoord.HasValue ? TertiaryTexCoord.Value.X : 0.0f);
            WriteNumberOfType(output, is16Bit, TertiaryTexCoord.HasValue ? TertiaryTexCoord.Value.Y : 0.0f);
