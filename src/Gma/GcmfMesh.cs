...
namespace LibGC.Gma
{
    public class GcmfMesh
    {
        ...
        internal void SaveHeader(EndianBinaryWriter output, bool isIndexed, bool is16Bit)
        {
            ...
            {
                // This field contains the number of integers instead of the number of bytes
                output.Write((Obj1StripsCcw.Count != 0) ? Obj1StripsCcw.NumIntsOfIndexed() : 0);
                output.Write((Obj1StripsCw.Count != 0) ? Obj1StripsCw.NumIntsOfIndexed() : 0);
            }
        ...
        internal void LoadIndexedData(EndianBinaryReader input, bool is16Bit, OrderedSet<GcmfVertex> vertexPool, HeaderSectionInfo headerSectionInfo)
        {
            if ((headerSectionInfo.SectionFlags & 0x01) != 0 && headerSectionInfo.Chunk1Size == 0)
            ...
            if ((headerSectionInfo.SectionFlags & 0x01) != 0)
                Obj1StripsCcw.LoadIndexed(input, is16Bit, headerSectionInfo.Chunk1Size, vertexPool, headerSectionInfo.VertexFlags);
            ...
            if ((headerSectionInfo.SectionFlags & 0x02) != 0)
                Obj1StripsCw.LoadIndexed(input, is16Bit, headerSectionInfo.Chunk2Size, vertexPool, headerSectionInfo.VertexFlags);
        ...
        internal int SizeOfIndexedData(bool is16Bit)
        {
            int size = 0;

            if (Obj1StripsCcw.Count != 0)
            {
                size += Obj1StripsCcw.SizeOfIndexed(is16Bit);
            }

            if (Obj1StripsCw.Count != 0)
            {
                size += Obj1StripsCw.SizeOfIndexed(is16Bit);
            }

            return size;
        }

        internal void SaveIndexedData(EndianBinaryWriter output, bool is16Bit, Dictionary<GcmfVertex, int> vertexPool)
        {
            if (Obj1StripsCcw.Count != 0)
            {
                Obj1StripsCcw.SaveIndexed(output, is16Bit, vertexPool);
            }

            if (Obj1StripsCw.Count != 0)
            {
                Obj1StripsCw.SaveIndexed(output, is16Bit, vertexPool);
            }
        }
